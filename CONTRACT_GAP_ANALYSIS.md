# VORA: where the design and the API contract disagree
...
`💬 Message` maps cleanly onto Phase 5.5 canned messages and needs a template picker screen (4 fixed options, no free text).
...
- **Booking:** a mode toggle (Exclusive / Shared) and a seats stepper (1 to 3).
...
| **border `#a2bbb3` on surface** | **2.04:1** | **FAIL** (needs 3:1 for UI components) | none |
