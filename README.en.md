...
<p align="center">
  <em>The opening screen, both fares side by side, and the pickup PIN. Every
  screenshot is the running app against the seeded database:
  <a href="docs/screenshots/">the full set</a>.</em>
</p>
...
173 unit tests. They need neither Postgres nor Redis.
...
That runs sixteen suites in order, ending with the security audit and the
...
driver KYC gate; a 5454-landmark gazetteer resolving at a 90.5 percent top-3
...
|---|---|
| unit tests | 173 |
| acceptance suites | 16, each passing |
| corridor economics | 46 checks, two bookings sharing one vehicle |
| ride needs chain | 9 checks, from the form to the driver's screen |
...
A note on running all sixteen back to back. Every login costs two argon2 hashes
